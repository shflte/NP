...
void motd(struct client *cli, char *buf) {
	...
	response(buf, "421", cli->name, "CAP :Unknown command");
					...
					// client disconnected
						...
						if (strncmp(parsed, "NICK", 4) == 0) {
							...
							for (int m = 0; m < 1000; m++) {
								if (strcmp(chalist[m].name, parsed) == 0) { // found
									found = 1;
									break;
								}
							}
							if (!found) {
								response(buf, "403", clilist[i].name, ":No such channel");
								continue;
							}
							bzero(buf, sizeof(buf));
							...
							found = 0;
							for (int m = 0; m < 1000; m++) {
								if (strcmp(chalist[m].name, cha) == 0) { // found
									found = 1;
									break;
								}
							}
							...
							found = 0;
							for (int m = 0; m < 1000; m++) {
								if (strcmp(chalist[m].name, cc) == 0) { // found
									found = 1;
									break;
								}
							}
							...
							if (parsed != NULL && msg != NULL) {
								de_r_n(msg);
								char temp[50];
							}
							else {
								bzero(buf, sizeof(buf));
								response(buf, "412", clilist[i].name, ":No text to send");
